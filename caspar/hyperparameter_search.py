...
def objective(trial, x, y, x_val, y_val, epochs=100, batch_size=32):
    ...
    momentum_rate = trial.suggest_float('momentum_rate', 0.1, 0.9, log=True)
    ...
    optimizer = tf.keras.optimizers.legacy.SGD(learning_rate=learning_rate, momentum=momentum_rate, nesterov=True)
    ...
    with mlflow.start_run(nested=True):
        ...
        for filename in os.listdir(checkpoint_dir):
            if filename.endswith(".h5"):
                ...
                if f"val_loss_{best_val_loss:.4f}" in filename:
...
def run_hyperparameter_optimization(x, y, x_val, y_val, n_trials=100, epochs=100,
                                    batch_size=32, experiment_name=None, n_jobs=-1):
    ...
    func = lambda _trial: objective(_trial, x, y, x_val, y_val, epochs, batch_size)
...
def optimize_architecture(args, x, y, x_val, y_val, n_jobs = -1, n_trials=100):
